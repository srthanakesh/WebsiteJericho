...
# Reload all images to get new vibrant contrast textures
...
# Remove all existing Jericho component objects
...
# Helper for PBR textured cards with Mix Shader Transparency
def make_pbr_card_mat(name, diffuse_file, rough_file=None, metal_file=None):
    ...
    links.new(mix.inputs[1], trans.outputs['BSDF'])
    links.new(mix.inputs[2], bsdf.outputs['BSDF'])
    ...
    if rough_file and os.path.exists(rough_file):
        ...
        tr.image.reload()
        links.new(bsdf.inputs['Roughness'], tr.outputs['Color'])
    else:
    ...
    if metal_file and os.path.exists(metal_file):
        ...
        tm.image.reload()
        links.new(bsdf.inputs['Metallic'], tm.outputs['Color'])
    else:
...
# 1. RIGHT SIDE: Speaker, Battery, PCB, Cable
create_pbr_card("Jericho_SpeakerModule_R", 0.0105, 0.0086, mat_spk)
create_pbr_card("Jericho_BatteryModule_R", 0.0185, 0.0067, mat_bat)
create_pbr_card("Jericho_SoCPCB_R", 0.0118, 0.0128, mat_pcb)
create_pbr_card("Jericho_FPC_Ribbon_R", 0.0168, 0.0083, mat_cab)

# 2. LEFT SIDE: Battery, PCB, Cable ONLY (NO Speaker, NO Camera!)
create_pbr_card("Jericho_BatteryModule_L", 0.0185, 0.0067, mat_bat)
create_pbr_card("Jericho_SoCPCB_L", 0.0118, 0.0128, mat_pcb)
create_pbr_card("Jericho_FPC_Ribbon_L", 0.0168, 0.0083, mat_cab)

# 3. CAMERA MODULE: STRICTLY ONLY ON RIGHT SIDE
...
create_pbr_card("Jericho_SensorPill_R", 0.0085, 0.0050, mat_pill)
...
print("SUCCESS: Scene created with vibrant textures and left-side breakdown (no camera, no speaker)!")
'''
